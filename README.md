...
在浏览器中打开全屏页即可（与主文件 `S5-Residency-Countdown.html` 一致）。**优先建议**用下面 **1**，不在内网时可用 **2** 或最下方「本地下载」到本地；**2** 仅作公网补充，不排在首选。
...
| 2 | **公网·Vercel（备选）** | <https://residency-countdown.vercel.app/> 由仓库在 **Vercel** 上部署，方便**不经过内网**时访问。<br>放在第二顺位的原因包括：**公网**链路与冷启动、边缘缓存等因素下，**偶发卡顿、首屏或动画不够顺滑**很常见；**免费档**在**流量/构建次数/带宽**等上也有**配额限制**。页面依赖的 **Google Fonts** 等也要走公网拉取。若你追求**最稳、最省流量**，请仍用**内网**或**本地下载**后本地打开。 |

内网 **IP/端口** 若调整，以维护者通知为准；Vercel 的用量与可访问性以 [Vercel](https://vercel.com) 与项目仪表盘为准。
